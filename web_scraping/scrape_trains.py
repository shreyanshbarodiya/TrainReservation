...
coach_info = {'1A': [1, 'H', 18], '2A': [2, 'A', 48], '3A': [4, 'B', 64], 'SL': [10, 'S', 72], 'CC': [3, 'C', 67],
              '2S': [4, 'D', 108]}
...
for i in range(1, 5):
    page = http.request('GET', url + "list?page=" + str(i))
        ...
        for class_ in classes:
            if class_ in coach_info.keys():
                for j in range(1, coach_info[class_][0] + 1):
                    query = "INSERT INTO coach VALUES('{}','{}','{}','{}');".format(train['trnno'],
                                                                                    coach_info[class_][1] + str(j),
                                                                                    class_, coach_info[class_][2])
                    try:
                        cur.execute(query)
                        print(query)
                    except Exception as inst:
                        ...
                        continue
                        # try:
        page_ = http.request('GET', url + train['trnno'])
        # except Exception as inst:
        # sys.stderr.write("Unable to get schedule {} {}\n".format(train['trnno'], inst))
        # continue

        soup_ = BeautifulSoup(page_.data, "html.parser")
        table_ = soup_.find_all('table', class_="results")
        if len(table_) < 2:
            continue
        schedule = {}
        table__ = table_[1]
        for tr_ in table__.find_all('tr'):
            td = tr_.find_all('td')
            ...
            if schedule['arr'] is None:
                query = "INSERT INTO schedule VALUES('{}','{}',null,'{}','{}','{}','{}');".format(train['trnno'],
                                                                                                  schedule['stn'],
                                                                                                  schedule['dept'],
                                                                                                  schedule['count'],
                                                                                                  schedule['day'],
                                                                                                  schedule['dist'])
            elif schedule['dept'] is None:
                query = "INSERT INTO schedule VALUES('{}','{}','{}',null,'{}','{}','{}');".format(train['trnno'],
                                                                                                  schedule['stn'],
                                                                                                  schedule['arr'],
                                                                                                  schedule['count'],
                ...
                query = "INSERT INTO schedule VALUES('{}','{}','{}','{}','{}','{}','{}');".format(train['trnno'],
                                                                                                  schedule['stn'],
                                                                                                  schedule['arr'],
                                                                                                  schedule['dept'],
                                                                                                  schedule['count'],
